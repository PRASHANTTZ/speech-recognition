...
gmmdir=/home/ps/kaldi/egs/timit/s5/exp/tri3
...
if [ ! -f $working_dir/sgmm.done ]; then
  ...
  steps/decode_sgmm2.sh --nj 8 --cmd "$decode_cmd" --acwt 0.125 --scoring-opts "$scoring_opts"  \
...
if [ ! -f $working_dir/mmi.sgmm.done ]; then
	steps/align_sgmm2.sh --nj 8 --cmd "$train_cmd" \
  ...
  steps/make_denlats_sgmm2.sh --nj 8 --beam 9.0 --lattice-beam 6 --cmd "$decode_cmd" $denlats_param \
